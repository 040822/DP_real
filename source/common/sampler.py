...
import numpy as np
import numba
from source.common.replay_buffer import ReplayBuffer
...
def create_indices(
    ...
    episode_len = np.diff(np.concatenate(([0], episode_ends)))
    print("[Dataset] Episode Length max:", np.max(episode_len), 
          "min:", np.min(episode_len), 
          "mean:", np.mean(episode_len))
